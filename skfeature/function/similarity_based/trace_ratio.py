...
def trace_ratio(X, y, n_selected_features, **kwargs):
    ...
    # fix:
    d = np.sum(s_within[0:n_selected_features])
    if d < 1:
        d = 1

    k = np.sum(s_between[0:n_selected_features])/d
    ...
    while True:
        ...
        if abs(k - old_k) < 1e-3:
            break
        # traceratio should be finished within very few iterations, if not,
        # something is wrong, e.g. k is inf.
        if count > 32:
            break
